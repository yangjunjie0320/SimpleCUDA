...
void kernel_ref(xt::xarray<float>& out, const xt::xarray<float>& inp) {
    xt::xarray<float> ai_sum = xt::sum(inp, {1}, xt::keep_dims);
    out = ai_sum;
}
}  // namespace block_reduce
  ...
  public:
    ...
    KernelLaunchConfig(kernel_t kernel, const char* title, dim3 block_dim, dim3 grid_dim,
        ...
        : kernel(kernel), title(title), warmup(10), repeat(100), block_dim(block_dim),
          grid_dim(grid_dim), shared_mem_size(shared_mem_size), tol(1e-4) {}

    KernelLaunchConfig(kernel_t kernel, const char* title, dim3 block_dim, dim3 grid_dim)
        : kernel(kernel), title(title), warmup(10), repeat(100), block_dim(block_dim),
          grid_dim(grid_dim), shared_mem_size(0), tol(1e-4) {}
    ...
    BenchmarkResult run(const xt::xarray<float>& inp) {
        ...
        xt::xarray<float> out_ref = xt::zeros<float>({nrow, 1});
        block_reduce::kernel_ref(out_ref, inp);

        xt::xarray<float> out_cpu = xt::zeros<float>({nrow, 1});
        xt::xarray<float> out_gpu = xt::zeros<float>({nrow, 1});
        ...
        for (int count = 0; count < repeat; count++) {
            block_reduce::kernel_cpu(out_cpu_ptr, inp_cpu_ptr, nrow, ncol);
        }
        auto t1_cpu = hrc::now();
        ...
        cudaMalloc(&out_gpu_ptr, nrow * sizeof(float));
        ...
        cudaMemcpy(out_gpu.data(), out_gpu_ptr, nrow * sizeof(float), cudaMemcpyDeviceToHost);
