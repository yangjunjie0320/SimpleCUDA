...
int main() {
    for (int nrow = 256; nrow <= 8192; nrow *= 2) {
        ...
        auto config = KernelLaunchConfig(softmax::kernel_v1, "softmax_f32_v1", block_dim, grid_dim);
        ...
        block_dim = dim3(NUM_THREAD_IN_WARP, NUM_WARP_IN_BLOCK);
        config = KernelLaunchConfig(softmax::kernel_v3, "softmax_f32_v3", block_dim, grid_dim);
        result = config.run(inp);
        result.print(false, true);
        ...
        config = KernelLaunchConfig(softmax::kernel_v5, "softmax_f32_v5", block_dim, grid_dim);
        result = config.run(inp);
        result.print(false, true);
