...
class KernelLaunchConfig {
  public:
    ...
    KernelLaunchConfig(kernel_t kernel, const char* kernel_name, size_t warmup, size_t repeat,
                       dim3 block_dim, dim3 grid_dim, size_t shared_mem_size)
        : kernel(kernel), kernel_name(kernel_name), warmup(warmup), repeat(repeat),
          block_dim(block_dim), grid_dim(grid_dim), shared_mem_size(shared_mem_size) {}
    ...
    BenchmarkResult run(torch::Tensor& inp) {
        ...
        auto launch = [&](float* out_gpu, const float* inp_gpu) {
            void* args[] = {(void*)&out_gpu, (void*)&inp_gpu, (void*)&nrow, (void*)&ncol};
        ...
        auto result = BenchmarkResult{time_cpu_ms, time_gpu_ms, err, kernel_name};
        return result;
    }
};
